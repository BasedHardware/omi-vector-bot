...
In a Handoff thread or a public help-forum post, `/done` posts a closed-ticket card with the Omi logo, then archives it. Named staff (`STAFF_USER_IDS` / `STAFF_ROLE_ID`) or anyone with Manage Threads. In `#vector-test`, anyone can `/done` if that list is empty. Vector never auto-closes from GitHub, from a community reply, or in bulk. Set `HELP_FORUM_CHANNEL_ID` only when you want Vector to answer in that forum. Order, email, and privacy still go to a private Handoff.
